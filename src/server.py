...
with socket()as main_socket:
 ...
 while 1:
  ...
  for new_socket in readable:
   if socket_list[0]==new_socket:socket_list+=new_socket.accept()[:1];guess_number=guess_number if socket_list[2:]else randint(1,100)
   else:
    new_recv=new_socket.recv(5)
    if new_recv:
     received_num,received_char=unpack("is",new_recv)
     if received_char==b"=":
      if received_num-guess_number:new_socket.send(b"K");socket_list.remove(new_socket)
      else:
